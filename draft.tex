...
(\textbf{Jordano2007?}). The persistence, functioning, and stability of
ecosystems emerge from these interactions, but antropogenic change
threatens to unravel and ``rewire'' these networks
(\textbf{CaraDonna2017IntRew?}). Plant-pollinator networks face two
...
and interactions across several field site, each consisting of several
plots across an elevational gradient, combined with spatial records of
species occurrence via GBIF to forecast this uncoupling.

Addresses the EBV to forecast of EBV element of the flow from data to
mitigation in fig.~\ref{fig:thesis}.
...
So we also need to predict interactions as they exist \emph{in the
present} as the data we have and temporally sparse and likely to contain
many interaction ``false-negatives'' (Strydom \emph{et al.} 2021).
...
landscape, represented as a raster (fig.~6, left). We then compute a new
resistance surface based on the proposed landscape modification, and
based on the values of resistance to dispersal between each location we
simulate spatially-explicit metapopulation dynamics model (Hanski \&
Ovaskainen 2000; Ovaskainen \emph{et al.} 2002) to estimate a
...
used for the preceding chapters.

Indeed \texttt{MetacommunityDynamics.jl} is the center of this, but due
to the nature of the Julia language, it is interoperable with serveral
existing packages.

\texttt{EcoJulia} organization, including several which I have
contributed to enable interoperability with MCD.jl.
...
\caption{todo}\label{fig:software}
}
\end{figure}
